...
fileprivate enum SpotifyQuery: String, URLConvertible {
    ...
    static func playlistUrlFor(playlistId: String) -> URL? {
        return URL(string: master.rawValue + playlists.rawValue + playlistId + "/tracks")
    }
...
public class SpotifyManager {
    ...
    public func save(trackId: String,
        ...
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.libraryUrlFor(SpotifyTrack.self),
                                      method: .PUT,
                                      parameters: self.libraryTrackIdsParameters(for: trackId),
                                      headers: self.authorizationHeader(with: token))
            { result in
    ...
    /**
     ...
     brings the saving success as parameter
     */
    public func addToPlaylist(trackId: String,
                              playlistId: String,
                              completionHandler: @escaping (Bool) -> Void) {
        tokenQuery { token in
            URLSession.shared.request(SpotifyQuery.playlistUrlFor(playlistId: playlistId)?.with(parameters: self.playlistTrackIdsParameters(for: trackId)),
                                      method: .POST,
                                      parameters: self.playlistTrackIdsParameters(for: trackId),
                                      headers: self.authorizationHeader(with: token))
            { result in
                if case .success(_) = result {
                    completionHandler(true)
    ...
    private func authorizationParameters(for application: SpotifyDeveloperApplication) -> HTTPRequestParameters {
        return [SpotifyParameter.clientId: application.clientId,
                ...
                SpotifyParameter.scope: SpotifyScope.string(with: [.readPrivate, .readEmail, .libraryModify, .libraryRead, .playlistModifyPublic, .playlistModifyPrivate])]
    }
     ...
     */
    private func libraryTrackIdsParameters(for trackId: String) -> HTTPRequestParameters {
        return [SpotifyParameter.ids: trackId]
    }
    
    /**
     Builds parameters for saving a track into a playlist
     - return: parameters for track saving
     */
    private func playlistTrackIdsParameters(for trackId: String) -> HTTPRequestParameters {
        return [SpotifyParameter.uris: trackId]
    }
